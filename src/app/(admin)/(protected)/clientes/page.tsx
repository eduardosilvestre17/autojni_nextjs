...
interface ICliente {
  ...
  phone1: string; // Vamos chamar de "Telemovel" na tabela
  phone2: string;
  mobilephone: string;
  fax: string;
  email: string;
 ...
 * ignorando acentos e case. Faz match parcial.
 */
...
export default function ClientesPage() {
  ...
  // Um único campo de pesquisa
  // que filtra por name, phone1, customertaxid
  ...
  // Debounce de 300ms para searchTerm
  useEffect(() => {
    const timer = setTimeout(() => {
  ...
  // Filtramos localmente por nome, telemovel, contribuinte
  const filteredClients = useMemo(() => {
    ...
    return allClients.filter((cliente) => {
      ...
      // Se der match em QUALQUER um dos 3 campos, já incluímos
  ...
  const columns = [
    ...
    { key: "phone1", label: "Telemovel" }, // Renomeado
  ];
    ...
    <div className="container max-w-5xl mx-auto px-4 py-6 bg-white rounded shadow dark:bg-gray-800">
      ...
      <div className="mb-4 p-4 bg-gray-100 dark:bg-gray-700 rounded">
        ...
        <div className="flex justify-end">
          <SecondaryButton type="button" onClick={handleLimpar}>
            Limpar
          </SecondaryButton>
        </div>
      </div>
